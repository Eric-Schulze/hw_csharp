...
namespace kMean_IrisDataset.cs
{
    class Program
    {
        ...
        private static List<string> listOfAttributes = new List<string>();

        static void Main(string[] args)
        {
            listOfAttributes.Add("petal_length");
            listOfAttributes.Add("petal_width");
            listOfAttributes.Add("sepal_length");
            listOfAttributes.Add("sepal_width");

            List<iris_datapoint> lid = readData();
            List<centroid> lc = populateCentroids(lid);

            lc = kMeans(ref lid, lc, 10000);
        ...
        public static List<centroid> populateCentroids(List<iris_datapoint> lid)
        {
            ...
            lc.Add(new centroid(pl_min, pw_min, sl_min, sw_min, 0));
            lc.Add(new centroid(pl_max, pw_max, sl_max, sw_max, numberOfClusters - 1));
            ...
            for (int i = 1; i < (numberOfClusters - 1); i++)
            {
                ...
                double sl_i = 0;
                double sw_i = 0;
                if (listOfAttributes.Contains("petal_length"))
                {
                    pl_i = pl_min + (i * ((pl_max - pl_min) / (numberOfClusters - 1)));
                }
                if (listOfAttributes.Contains("petal_width"))
                {
                    pw_i = pw_min + (i * ((pw_max - pw_min) / (numberOfClusters - 1)));
                }
                if (listOfAttributes.Contains("sepal_length"))
                {
                    sl_i = sl_min + (i * ((sl_max - sl_min) / (numberOfClusters - 1)));
                }
                if (listOfAttributes.Contains("sepal_width"))
                {
                    sw_i = sw_min + (i * ((sw_max - sw_min) / (numberOfClusters - 1)));
                }
                lc.Add(new centroid(pl_i, pw_i, sl_i, sw_i, i));
            }
            return lc;
        }

        private static double calculateMultidimensionalDistance(double d1 = 0, double c1 = 0, double d2 = 0, double c2 = 0, double d3 = 0, double c3 = 0, double d4 = 0, double c4 = 0)
        {
        ...
        private static List<centroid> kMeans(ref List<iris_datapoint> lid, List<centroid> lc, int limit)
        {
            ...
            while (!noChange && counter < limit)
            {
                ...
                foreach (iris_datapoint id in lid)
                {
                    foreach (centroid c in lc)
                    {
                        double c1 = 0, c2 = 0, c3 = 0, c4 = 0, d1 = 0, d2 = 0, d3 = 0, d4 = 0;
                        if (listOfAttributes.Contains("petal_length"))
                        {
                            d1 = id.petal_length;
                            c1 = c.petal_length;
                        ...
                        c.tempDistance = calculateMultidimensionalDistance(d1, c1, d2, c2, d3, c3, d4, c4);
                    }
